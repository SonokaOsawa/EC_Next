...
import { Carttype, deleteCartitem, selectCart, Iteminfotype, setCart, getCart } from '../../features/cart';
...
export const CartItems: FC<Props> = ({items}) => {
    ...
    const deleteBtn = (iteminfo: Iteminfotype) => {
        if(user.uid) {
            ...
            newcart.iteminfo = cart.iteminfo.filter((item) => item.id !== iteminfo.id)
